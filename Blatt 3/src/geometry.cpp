...
Geometry::Geometry(const Communicator *comm) {
    ...
    Load("default.geom_drivencavity_small");
    _comm = comm;
        ...
        if (strcmp(name,"geometry") == 0) {
            if (fscanf(handle," %s\n", name))
                ...
                if(strcmp(name,"free") == 0) {
                    ...
                    //_flag = _allflag; //only for #processes = 1 else segfault
//                     const multi_index_t real_size = multi_index_t((_bsize[0])/_comm->ThreadDim()[0],(_bsize[1])/_comm->ThreadDim()[1]) ;
//                     for (int k = 0; k< _size[0];k++){
//                         for (int l = 0; l< _size[1];l++){
//                             _flag[l*_size[0] + k] = _allflag[(_comm->ThreadIdx()[0]-1)*real_size[0] + (_comm->ThreadIdx()[1]-1)*real_size[1] + l*(_bsize[0]-_size[0]) + k] 
//                         }
//                     }
